...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, toptitle=1mm, toprule=.15mm, rightrule=.15mm, arc=.35mm, bottomrule=.15mm, opacityback=0, colback=white, breakable, left=2mm, opacitybacktitle=0.6, bottomtitle=1mm, leftrule=.75mm, titlerule=0mm, colframe=quarto-callout-tip-color-frame, coltitle=black, title={STRidge algorithm for sparse representation (Julia implementation)}]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, toptitle=1mm, toprule=.15mm, rightrule=.15mm, arc=.35mm, bottomrule=.15mm, opacityback=0, colback=white, breakable, left=2mm, opacitybacktitle=0.6, bottomtitle=1mm, leftrule=.75mm, titlerule=0mm, colframe=quarto-callout-tip-color-frame, coltitle=black, title={STRidge algorithm extension to multidimensional observations (Julia
implementation)}]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, toptitle=1mm, toprule=.15mm, rightrule=.15mm, arc=.35mm, bottomrule=.15mm, opacityback=0, colback=white, breakable, left=2mm, opacitybacktitle=0.6, bottomtitle=1mm, leftrule=.75mm, titlerule=0mm, colframe=quarto-callout-tip-color-frame, coltitle=black, title={Compute derivatives of a trajectory (Julia implementation)}]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-note-color!10!white, toptitle=1mm, toprule=.15mm, rightrule=.15mm, arc=.35mm, bottomrule=.15mm, opacityback=0, colback=white, breakable, left=2mm, opacitybacktitle=0.6, bottomtitle=1mm, leftrule=.75mm, titlerule=0mm, colframe=quarto-callout-note-color-frame, coltitle=black, title={SINDy {[}standard{]}}]
...
\KeywordTok{function} \FunctionTok{integrate\_me!}\NormalTok{(du, u, p, t)}
  \CommentTok{\# ode\_order = 2, with 3 library functions}
...
would correspond to a term \(y'' = x (z')^2\) in the original ODE. This
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, toptitle=1mm, toprule=.15mm, rightrule=.15mm, arc=.35mm, bottomrule=.15mm, opacityback=0, colback=white, breakable, left=2mm, opacitybacktitle=0.6, bottomtitle=1mm, leftrule=.75mm, titlerule=0mm, colframe=quarto-callout-tip-color-frame, coltitle=black, title={Generate a function applying library functions to an integrand (Julia
implementation)}]
...
\NormalTok{    du[}\FloatTok{1}\OperatorTok{:}\NormalTok{n\_vars}\OperatorTok{*}\NormalTok{ode\_order] }\OperatorTok{.=}\NormalTok{ u[ode\_order}\OperatorTok{+}\FloatTok{1}\OperatorTok{:}\FunctionTok{n\_vars*}\NormalTok{(ode\_order}\OperatorTok{+}\FloatTok{1}\NormalTok{)] }
\NormalTok{    i }\OperatorTok{=}\NormalTok{ n\_vars}\OperatorTok{*}\NormalTok{ode\_order}\OperatorTok{+}\FloatTok{1}\OperatorTok{:}\FunctionTok{n\_vars*}\NormalTok{(ode\_order}\OperatorTok{+}\FloatTok{1}\NormalTok{)}
    \ControlFlowTok{for}\NormalTok{ j }\OperatorTok{=} \FloatTok{1}\OperatorTok{:}\FunctionTok{length}\NormalTok{(library)}
\NormalTok{      du[i[j]] }\OperatorTok{=}\NormalTok{ library[j](t, u[}\FloatTok{1}\OperatorTok{:}\NormalTok{n\_vars}\OperatorTok{*}\NormalTok{ode\_order])}
    \ControlFlowTok{end}
  \KeywordTok{end}
\KeywordTok{end}
\end{Highlighting}
...
coordinates, their derivatives, and arbitrary user-defined functions.

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, toptitle=1mm, toprule=.15mm, rightrule=.15mm, arc=.35mm, bottomrule=.15mm, opacityback=0, colback=white, breakable, left=2mm, opacitybacktitle=0.6, bottomtitle=1mm, leftrule=.75mm, titlerule=0mm, colframe=quarto-callout-tip-color-frame, coltitle=black, title={Compute arbitrary polynomial functions of variables and their
derivatives (Julia implementation)}]
...
\begin{verbatim}
5×15 Matrix{Float64}:
 1.0  0.421  -2.46   0.571  -4.41   0.177  …  10.8    0.326  -2.52   19.4
 1.0  0.346   3.02   0.292   1.81   0.12       5.46   0.085   0.527   3.27
 1.0  0.566   3.15   0.474   3.2    0.321     10.1    0.225   1.52   10.3
 1.0  0.472  -2.04   0.579  -0.313  0.223      0.637  0.335  -0.181   0.0979
 1.0  0.344   0.459  0.679   0.594  0.119      0.273  0.461   0.403   0.352
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# function lorenz!(du, u, p, t)}
\CommentTok{\#     x, y, z = u}
\CommentTok{\#     du[1] = 10.0 * (y {-} x)}
\CommentTok{\#     du[2] = x * (28.0 {-} z) {-} y}
\CommentTok{\#     du[3] = x * y {-} (8 / 3) * z}
\CommentTok{\# end}

\CommentTok{\# sol\_lorenz = ODEProblem(lorenz!, [1.0, 1.0, 1.0], (0.0, 10.0)) |\textgreater{} solve}

\CommentTok{\# times\_lorenz = range(0.0, 10.0, length = 100) |\textgreater{} collect}
\CommentTok{\# traj\_lorenz = [sol\_lorenz(t)[i] for t in times\_lorenz, i = 1:3]}

\CommentTok{\# ds = derivatives(times\_lorenz, traj\_lorenz, derivative\_order = 1)}
\CommentTok{\# x\_L, dx\_L = ds[1][:,1], ds[2][:,1]}
\CommentTok{\# y\_L, dy\_L = ds[1][:,2], ds[2][:,2]}
\CommentTok{\# z\_L, dz\_L = ds[1][:,3], ds[2][:,3]}

\CommentTok{\# target\_data = [dx\_L ;; dy\_L ;; dz\_L]}
\CommentTok{\# library\_data, library\_names = polynomials(traj\_lorenz, 3, var\_names = ["x", "y", "z"])}

\CommentTok{\# rr = STRidge(target\_data, library\_data, lambda\_sparse = 0.3)}
\CommentTok{\# pretty\_print(rr, library\_names)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# points = []}
\CommentTok{\# rrs = []}
\CommentTok{\# n\_terms\_poss = []}
\CommentTok{\# for lambda\_sparse in 10 .\^{} range({-}3, 1, length = 1000)}
\CommentTok{\#   rr = STRidge(target\_data, library\_data, lambda\_sparse = lambda\_sparse, lambda\_ridge = 0.1)}
\CommentTok{\#   rmse = sqrt(mean(abs2.(target\_data {-} library\_data * rr)))}
\CommentTok{\#   n\_terms = count(iszero, rr)}

\CommentTok{\#   if !(n\_terms in n\_terms\_poss)}
\CommentTok{\#     push!(points, [lambda\_sparse, n\_terms, rmse])}
\CommentTok{\#     push!(rrs, rr)}
\CommentTok{\#     push!(n\_terms\_poss, n\_terms)}
\CommentTok{\#   else}
\CommentTok{\#     points[end] = [lambda\_sparse, n\_terms, rmse]}
\CommentTok{\#     rrs[end] = rr}
\CommentTok{\#   end}
\CommentTok{\# end}
\CommentTok{\# points = stack(points, dims = 1)}
\CommentTok{\# scatter(log10.(points[:,1]), points[:,3], color = points[:,2])}
\end{Highlighting}
\end{Shaded}
