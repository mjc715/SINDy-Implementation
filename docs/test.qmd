...
We can therefore treat the data generated by `derivatives` as the `target_data` for our sparse representation algorithm; this is precisely the standard SINDy algorithm. There is one more detail to consider, namely that it is cumbersome to create lists of every possible multivariable monomial by hand. We therefore automate this process using the `polynomials` function below.
