...
```
Hence, a function like `f2(t, u) = u[1]*u[6]^2` would correspond to a term $y'' = x (z')^2$ in the original ODE. This suggests that the final library functions we create should be of the form `f(t, u)`. Generalizing this pattern, we now define a simple helper function to build this for us, given our library functions and order.
...
function library_to_odefun(library::Vector{<:Function}, ode_order::Integer)
  ...
  function odefun!(du, u, p, t)
    du[1:n_vars*ode_order] .= u[ode_order+1:n_vars*(ode_order+1)] 
    i = n_vars*ode_order+1:n_vars*(ode_order+1)
    for j = 1:length(library)
      du[i[j]] = library[j](t, u[1:n_vars*ode_order])
    end
  end
end
```
...
The most common library functions are polynomials, but it is cumbersome to create lists of every possible multivariable monomial by hand. We therefore automate this process using the `polynomials` function below. 
