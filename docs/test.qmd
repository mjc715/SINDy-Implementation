...
\end{align}
\end{subequations}
where $X(t) = (x(y), y(t), z(t),  \dots))$ is a vector of coordinates and $f_1$, $f_2$ can be represented by sparse linear combinations of elementary functions. We also assume that the data we have access to are trajectory observations $\{X(t_1), X(t_2), \dots \}$. Note that if we had access to $\{\dot{X}(t_1), \dot{X}(t_2), \dots \}$ then this would immediately result in a sparse representation problem for the function $f_1$ and analogously for $\{\ddot{X}(t_1), \ddot{X}(t_2), \dots \}$ and $f_2$. Hence, we only need one preprocessing function `derivatives` outlined below which will provide the required derivatives via interpolation.

::: {.callout-tip icon=false}
## Compute first and second derivatives of a trajectory (Julia implementation)
...
function derivatives(
  ...
  itps_gridded = [
    interpolate((times,), trajectories[:,i], Gridded(Linear())) # <3>
    for i = 1:size(trajectories, 2)]

  ts = range(times[1], times[end], length = length(times))
  itps = [
    cubic_spline_interpolation(ts, itps_gridded[i].(ts)) # <4>
    for i = 1:size(trajectories, 2)]

  data = [ # <5>
    trajectories,
    [gradient(itp, t)[1] for t in times, itp in itps],
    [hessian(itp, t)[1] for t in times, itp in itps]
  ]
...
3. We interpolate the (possibly non-uniform) observation times to a uniform grid. The functions `interpolate`, `cubic_spline_interpolation`, `gradient` and `hessian` are from [Interpolations.jl](http://juliamath.github.io/Interpolations.jl/latest/).
4. Construct cubic interpolants for each coordinate on the new uniform grid `ts`.
5. Construct a vector of three matrices, $X$, $\dot{X}$ and $\ddot{X}$, all evaluated at the original `times`.
:::

We can therefore treat the data generated by `derivatives` as the `target_data` for our sparse representation algorithm; this is precisely the standard SINDy algorithm.

::: {.callout-tip icon=false}
## SINDy [standard] (Julia implementation)
...
times_lorenz = range(0.0, 10.0, length = 100) |> collect
traj_lorenz = [sol_lorenz(t)[i] for t in times_lorenz, i = 1:3]

ds = derivatives(times_lorenz, traj_lorenz)
x_L, dx_L, d2x_L = ds[1][:,1], ds[2][:,1], ds[3][:,1]
y_L, dy_L, d2y_L = ds[1][:,2], ds[2][:,2], ds[3][:,2]
z_L, dz_L, d2z_L = ds[1][:,3], ds[2][:,3], ds[3][:,3]

target_data = [dx_L ;; dy_L ;; dz_L]

library_data = [
  ones(length(times_lorenz)) ;; 
  x_L ;; y_L ;; z_L ;; 
  x_L .^2 ;; y_L .^2 ;; z_L .^ 2 ;;
  x_L .* y_L ;; x_L .* z_L ;; y_L .* z_L]
...
rr = STRidge(target_data, library_data, lambda_sparse = 0.87)
