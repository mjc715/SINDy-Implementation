...
for lambda_sparse in 10 .^ range(-3, 1, length = 100)
  rr = STRidge(g_points, library_data, lambda_sparse = lambda_sparse, lambda_ridge = 0.1)
  rmse = sqrt(mean(abs2.(g_points - library_data * rr)))
...
```
1. `times` is a vector of times that the trajectory data are given on of length $N$.
...
We can therefore treat the data generated by `derivatives` as the `target_data` for our sparse representation algorithm; this is precisely the standard SINDy algorithm which we may now describe in pseudocode.
...
2. Compute `D = derivatives(t, X, derivative_order = n)`. The last entry of `D` is an $N \times v$ matrix of $\text{d}X^n/\text{d}t^n$ values, which is the `target_data`.
3. Construct `library_data` containing the proposed functions to include in the sparse representation. This is a matrix with $N$ rows and an arbirary number of columns. Each column is in principle any function of $t$ evaluated at each time. Usually the most appropriate functions of $t$ will be functions of the coordinates $v(t)$. Note that entries of `D` other than the last one are derivatives of order up to $n - 1$ of $X$ and any function of these may also appear in `library_data`.
4. Compute `Xi = STRidge(target_data, library_data; lambda_sparse, lambda_ridge)`, where `lambda_sparse` should be chosen small enough to sparsify the representation but large enough to faithfully represent $X$. The final sparse representation is given by multiplying `Xi` by the library functions.
:::

The most common library functions are polynomials, but it is cumbersome to create lists of every possible multivariable monomial by hand. We therefore automate this process using the `polynomials` function below. Note that this is not necessarily part of the core SINDy algorithm, it is simply a convenience.

::: {.callout-tip icon=false}
## Compute arbitrary polynomial functions of variables (Julia implementation)
...
function polynomials(
  ...
  var_names = var_names === nothing ? ["x$(i)" for i = 1:size(data, 2)] : var_names
  exps = [multiexponents(size(data, 2), i) for i = 0:order] # <4>
...
```
1. `data` is a matrix with $N$ rows and a number of columns equal to the number of variables.
2. `order` is an integer equal to the maximum monomial order requested, e.g. if `order == 2` with two variables `x, y`, then `polynomials` will compute `1, x, y, x^2, y^2, xy`.
...
6. If an exponent is zero, we don't want its name to show uup in that term, and if an exponent is 1, we omit the exponent. Otherwise, the exponent is shown.
:::

```{julia}
data = rand(3,  2) # 3 times, 2 variables
order = 2 # all monomials up to and including 2

ps = polynomials(data, order, var_names = ["x", "y"])
@info ps[2]
...
sol_lorenz = ODEProblem(lorenz!, [1.0, 1.0, 1.0], (0.0, 10.0)) |> solve

times_lorenz = range(0.0, 10.0, length = 1000) |> collect
...
library_data, library_names = polynomials(traj_lorenz, 3, var_names = ["x", "y", "z"])

rr = STRidge(target_data, library_data, lambda_sparse = 0.3)
pretty_print(rr, library_names)
```
...
for lambda_sparse in 10 .^ range(-3, 1, length = 1000)
  rr = STRidge(target_data, library_data, lambda_sparse = lambda_sparse, lambda_ridge = 0.1)
  rmse = sqrt(mean(abs2.(target_data - library_data * rr)))
  n_terms = count(iszero, rr)
...
points = stack(points, dims = 1)
scatter(log10.(points[:,1]), points[:,3], color = points[:,2])
```
